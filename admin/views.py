...
@requires_roles('admin')  # makes it so you need to be logged in with a user who has admin permissions
def admin():
...
def create_winning_draw():
    ...
    new_winning_draw = Draw(user_id=current_user.id, numbers=submitted_draw, master_draw=True, lottery_round=lottery_round)
...
def view_winning_draw():
    ...
    if current_winning_draw:
        current_winning_draw.decrypt_number(current_user.id)  # decrypts number
